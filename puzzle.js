...
class PuzzlePiece {
  constructor(row, col, length, isHorizontal, color) {
    ...
    this.color = color;
    this.isRedCar = false; // Flag for the target car that needs to exit
  }
}

// Define the red car (escape vehicle) as the first piece
const puzzlePieces = [
  new PuzzlePiece(2, 0, 2, true, 'red'), // Red car (escape vehicle)
  new PuzzlePiece(0, 1, 3, true, '#87CEEB'), // Light blue
  new PuzzlePiece(3, 1, 1, false, '#90EE90'), // Light green
  new PuzzlePiece(4, 1, 2, false, '#ADD8E6'), // Blue
  new PuzzlePiece(0, 4, 2, true, '#FFC0CB'), // Pink
  new PuzzlePiece(4, 5, 2, true, '#87CEEB'), // Light blue
  new PuzzlePiece(3, 5, 1, false, '#90EE90')  // Light green
];

// Mark the red car
puzzlePieces[0].isRedCar = true;
...
function initializePuzzle() {
  ...
  exit.style.top = `${2 * CELL_SIZE}px`; // Position at the row of the red car
  exit.style.width = '10px';
  ...
  puzzlePieces.forEach(piece => {
    ...
    pieceElement.addEventListener('mousedown', (e) => startDrag(e, piece, pieceElement));
...
function isValidMove(piece, newRow, newCol) {
  ...
  if (newRow < 0 || newRow + (piece.isHorizontal ? 0 : piece.length - 1) >= BOARD_SIZE) return false;
  if (newCol < 0 || newCol + (piece.isHorizontal ? piece.length - 1 : 0) >= BOARD_SIZE) return false;
  ...
  for (let otherPiece of puzzlePieces) {
    ...
    const otherEndRow = otherPiece.row + (otherPiece.isHorizontal ? 0 : otherPiece.length - 1);
    const otherStartCol = otherPiece.col;
    const otherEndCol = otherPiece.col + (otherPiece.isHorizontal ? otherPiece.length - 1 : 0);
    ...
    // Check for overlap
    if (pieceStartRow <= otherEndRow && pieceEndRow >= otherStartRow &&
        pieceStartCol <= otherEndCol && pieceEndCol >= otherStartCol) {
      return false;
    }
  }
